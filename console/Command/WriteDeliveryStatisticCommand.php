...
final class WriteDeliveryStatisticCommand extends BaseCommand
{
    private const KEY_PREFIX_STAT = 'swe:';
    private const SEPARATOR = '·';
    protected static $defaultName = 'write:delivery-statistic';
    ...
    {
        $this
            ->setDescription('Move delivery statistic from redis to MariaDB')
            ->addOption(
                'prev_intervals',
                'p',
    ...
    protected function executeCommand(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($keys as $key) {
            ...
            $insertData[] = [
                'date_time' => date("Y-m-d H:i:s", (int)substr($attrs[0], 4)),
            ...
            $attrs[0] = date("Y-m-d 00:00:00", (int)substr($attrs[0], 4));
            $insertDailyData[] = [
                'date_time' => $attrs[0],
                'unique_key' => $this->createUniqueKey($attrs),
                'partner_id' => (int)$attrs[1],
        ...
        while (count($insertData) > 0) {
            ...
            $sql = $this->insert('delivery_statistic_10min', $data, array_keys($data[0]));

            $res = $this->pdo->exec($sql);

            $this->logger->notice('inserted in delivery_statistic_10min rows: ' . count($data));
            if ($res === false) {
        ...
        while (count($insertDailyData) > 0) {
            ...
            $sql = $this->insert('delivery_statistic_daily', $data, array_keys($data[0]));
            $sql .= ' ON DUPLICATE KEY UPDATE `delivered` = `delivered` + VALUES(`delivered`),
                `clicked` = `clicked` + VALUES(`clicked`),
                `closed` = `closed` + VALUES(`closed`),
            ...
            $this->logger->notice('inserted or updated in delivery_statistic_daily rows: ' . count($data));
            if ($res === false) {
                throw new \RuntimeException($this->pdo->errorCode() . implode(', ', $this->pdo->errorInfo()));
            }
        }
